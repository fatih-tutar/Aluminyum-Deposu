<?php
	...
	$tekliflistesi = $db->query("SELECT * FROM teklif_listesi WHERE silik = '0' ORDER BY teklifid DESC")->fetchAll(PDO::FETCH_ASSOC);
  ...
  <body class="body-white">
        ...
        <div id="mainCol" class="col-md-10 col-12">
          <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
            <div class="d-flex align-items-center">
              ...
              <h3 class="d-none d-md-block mb-0 mt-2" style="font-weight: bold;">Teklif Listesi</h3>
            </div>
            <div class="d-flex gap-2">
              <?php if ($user->type == '2'): ?>
              <a href="/teklifarsivi" target="_blank" class="btn btn-outline-secondary btn-sm">
                <i class="fas fa-archive me-2"></i> Teklif Arşivi
              </a>
          ...
          <!-- Arşive ekle onay modalı -->
          <div id="arsivarti-modal" class="modal">
            <span class="close" onclick="closeModal()">&times;</span>
            <h5 class="mb-3"><b>Bu kaydı arşive eklemek istediğinize emin misiniz?</b></h5>
            <form action="" method="POST" class="mt-3">
              <input type="hidden" name="teklifid" id="arsivarti-teklifid" value="">
              <div class="d-flex justify-content-end gap-2">
                <button type="button" class="btn btn-secondary" onclick="closeModal()">Vazgeç</button>
                <button type="submit" name="arsivarti" class="btn btn-success">Arşive Ekle</button>
              </div>
            </form>
          </div>

          <!-- Arşivden çıkar onay modalı -->
          <div id="arsiveksi-modal" class="modal">
            <span class="close" onclick="closeModal()">&times;</span>
            <h5 class="mb-3"><b>Bu kaydı arşive göndermek istediğinize emin misiniz?</b></h5>
            <form action="" method="POST" class="mt-3">
              <input type="hidden" name="teklifid" id="arsiveksi-teklifid" value="">
              <div class="d-flex justify-content-end gap-2">
                <button type="button" class="btn btn-secondary" onclick="closeModal()">Vazgeç</button>
                <button type="submit" name="arsiveksi" class="btn btn-danger">Arşivden Çıkar</button>
              </div>
            </form>
          </div>

          <!-- Teklif Düzenleme Modalı (tek modal, JS ile doldurulacak) -->
                ...
                <tr>
                  ...
                  <td>
                    <div class="d-flex flex-wrap gap-1">
                      ...
                      <button type="button" class="btn btn-success btn-sm arsivarti-btn" title="Arşive ekle" data-teklifid="<?= (int)$teklifid ?>"><i class="fas fa-plus"></i></button>
                      <button type="button" class="btn btn-danger btn-sm arsiveksi-btn" title="Arşivden çıkar" data-teklifid="<?= (int)$teklifid ?>"><i class="fas fa-times"></i></button>
                    </div>
                  </td>
                </tr>
              <?php
    ...
    <script>
      (function() {
        ...
        document.querySelectorAll('.arsivarti-btn').forEach(function(btn) {
          btn.addEventListener('click', function() {
            document.getElementById('arsivarti-teklifid').value = this.dataset.teklifid || '';
            if (typeof openModal === 'function') openModal('arsivarti-modal');
          });
        });
        document.querySelectorAll('.arsiveksi-btn').forEach(function(btn) {
          btn.addEventListener('click', function() {
            document.getElementById('arsiveksi-teklifid').value = this.dataset.teklifid || '';
            if (typeof openModal === 'function') openModal('arsiveksi-modal');
          });
        });
      })();
    </script>
